...
println "EDMP CONFIG URL: ${EDMP_CONFIG_URL}"
...
def edmpGitUrl="https://github.com/omkar100100/event-driven-microservices-platform-master"
...
def createCIJob(def jobNamePrefix, def gitProjectName, def gitRepositoryUrl, def rootWorkDirectory) {
  ...
  job("${jobNamePrefix}-1-ci") {
    ...
    parameters {
      ...
      stringParam("REPOSITORY_URL", "http://192.168.99.100:8081/nexus/content/repositories/releases/", "Nexus Release Repository URL")
    }
    scm {
      git {
        remote {
